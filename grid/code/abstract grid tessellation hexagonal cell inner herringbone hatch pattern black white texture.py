...
def draw():
    """Honeycomb cells filled with mini herringbone hatch that flips direction."""
    fig, ax = setup_ax()
    r = 6.2
    ...
    for row in range(-2, 14):
        for col in range(-2, 14):
            ...
            ang = 40 if (row + col) % 2 == 0 else -40
            for k in range(-3, 4):
                ax.plot([-r * 0.55, r * 0.55], [k * 1.05, k * 1.05], color="black", linewidth=0.45,
                        transform=Affine2D().rotate_deg(ang).translate(cx, cy) + ax.transData)
    save(fig, "abstract grid tessellation hexagonal cell inner herringbone hatch pattern black white texture")
