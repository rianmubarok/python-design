...
for s in scripts:
    with open(s, 'r', encoding='utf-8') as f:
        content = f.read()
    content = content.replace('DATE = datetime.now().strftime("%d%m%Y")', 'DATE = "20092026"')
    with open(s, 'w', encoding='utf-8') as f:
        f.write(content)
    subprocess.run(['python', s], capture_output=True)
    content = content.replace('DATE = "20092026"', 'DATE = datetime.now().strftime("%d%m%Y")')
    with open(s, 'w', encoding='utf-8') as f:
        f.write(content)
    print('Generated 20092026:', s)