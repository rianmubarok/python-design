...
def lightning_bolt(ax, cx, cy, height, width, angle_deg=0, fill="white"):
    ...
    h = height
    w = width
    # Z-bolt vertices (pointing downward)
    pts = np.array([
        [-w * 0.10,  h * 0.50],   # top-left
        [ w * 0.50,  h * 0.50],   # top-right
        [ w * 0.05,  h * 0.04],   # mid-right inner
        [ w * 0.45,  h * 0.04],   # mid-right outer
        [ w * 0.10, -h * 0.50],   # bottom-right
        [-w * 0.50, -h * 0.50],   # bottom-left
        [-w * 0.05, -h * 0.04],   # mid-left inner
        [-w * 0.45, -h * 0.04],   # mid-left outer
    ])
    # rotate
...
def branching_bolt(ax, cx, cy, height, width, angle_deg=0, depth=2, rng=None):
    ...
    if depth > 0:
        for _ in range(rng.integers(1, 3)):
            ...
            # branch origin: somewhere along main bolt
