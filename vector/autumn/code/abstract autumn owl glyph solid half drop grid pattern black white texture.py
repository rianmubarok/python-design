...
def draw_owl_detailed(ax, cx, cy, size):
    ...
    # Peanut shape: 0.6 is base width, pinch at the neck
    ...
    # Eyebrows (V shape cutouts to make it look angry/wise)
    ...
    # Wings (white curved lines on the side)
...
def main():
    ...
    for i in range(-2, cols+3):
        for j in range(-2, rows+3):
            ...
            if i % 2 != 0: cy += spacing_y / 2
            draw_owl_detailed(ax, cx, cy, size=4.0)
    save(fig, "abstract autumn owl glyph solid half drop grid pattern black white texture")
