...
def abstract_grid_tessellation_isometric_rounded_cube_twist_wave_pattern_black_white_texture():
    """Tweak: Isometric cube grid where top face vertices twist according to a spiral function."""
    fig, ax = setup_ax()
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            ...
            # Twist angle
            twist = 0.4 * np.sin(cx * 0.1)
            
            v_top = (cx + twist * a, cy + a)
            v_center = (cx, cy)
    ...
    save(fig, "abstract grid tessellation isometric rounded cube twist wave pattern black white texture")


if __name__ == "__main__":
    abstract_grid_tessellation_isometric_rounded_cube_twist_wave_pattern_black_white_texture()
