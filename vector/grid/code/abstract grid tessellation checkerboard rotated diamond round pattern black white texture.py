...
def draw():
    """Checkerboard of rotated diamonds morphing into rounded squares."""
    fig, ax = setup_ax()
    n = 10
    cell = 100 / n
    for r in range(n):
        for c in range(n):
            ...
            t = c / (n - 1)
            rot = 45 * (1 - t)
            rnd = 0.15 + t * cell * 0.28
            w = cell * 0.78
            ax.add_patch(FancyBboxPatch((-w / 2, -w / 2), w, w,
                                        boxstyle=f"round,pad=0,rounding_size={rnd}",
                                        facecolor="black" if (r + c) % 2 == 0 else "none",
                                        edgecolor="black", linewidth=1.05,
                                        transform=Affine2D().rotate_deg(rot).translate(cx, cy) + ax.transData))
    save(fig, "abstract grid tessellation checkerboard rotated diamond round pattern black white texture")
