...
def draw():
    """Skull grid on rotated 45° diamond layout — skulls sit on diamond nodes."""
    fig, ax = setup_ax()
    step = PERIOD / 7.0
    s = step * 0.72
    cols, rows = 9, 9
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            cy = (row + 0.5)*dy
            for ox, oy in WRAPS:
                skull(ax, cx+ox, cy+oy, s * 0.58)
    save(fig, "abstract halloween variation skull diamond rhombus grid layout pattern black white texture")
