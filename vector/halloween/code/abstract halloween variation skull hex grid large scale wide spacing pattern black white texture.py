...
def skull(ax, cx, cy, s, fill="black"):
    """Large skull with exaggerated oversized eye sockets."""
    ...
    for ang in (42, -42):
        ...
        ax.add_patch(FancyBboxPatch(
            ...
            facecolor=fill, edgecolor=edge, linewidth=elw, transform=tr))
...
def draw():
    """Hexagonal grid with larger skulls and wide open spacing (4 cols).
    Fewer skulls per tile, more breathing room between them.
    Alternating black/white fill per hex cell.
    """
    fig, ax = setup_ax()

    cols  = 4                              # fewer cols = bigger cells
    hex_w = PERIOD / cols                  # ~25 units between centres
    hex_h = hex_w * (np.sqrt(3) / 2)
    rows  = int(np.ceil(PERIOD / hex_h)) + 2
    s     = hex_w * 0.44                  # skull fills ~88% of cell
    ...
    save(fig,
         "abstract halloween variation skull hex grid large scale wide spacing "
