...
def draw_crescent(ax, cx, cy, r_outer, tilt, fill="black"):
    """Single crescent with configurable fill colour."""
    n_pts = 200
...
def concentric_crescents_deep(ax, cx, cy, max_r, n=6, tilt=0.0):
    """6-layer nested crescents, alternating black/white fill for depth effect."""
    for i in range(n):
        scale = (n - i) / n
        r = max_r * scale
        shift = (max_r - r) * 0.32
        sx = cx + shift * np.cos(tilt)
        sy = cy + shift * np.sin(tilt)
        fill = "black" if i % 2 == 0 else "white"
        draw_crescent(ax, sx, sy, r, tilt, fill=fill)


def draw():
    """3×4 grid, each cell has 6-layer nested crescents alternating black/white.
    White background, tight radial spacing for a layered ring cake look.
    """
    fig, ax = setup_ax()
    cols, rows = 3, 4
    dx, dy = PERIOD / cols, PERIOD / rows
    max_r = min(dx, dy) * 0.42

    tilts = [np.radians(t) for t in [15, 345, 60, 300, 30, 330, 45, 315, 20, 340, 55, 305]]
    idx = 0
    for row in range(rows):
        for col in range(cols):
            ...
            idx += 1
            for ox, oy in WRAPS:
                concentric_crescents_deep(ax, cx + ox, cy + oy, max_r, n=6, tilt=tilt)

    save(fig,
         "abstract halloween variation crescent moon deep concentric 6 layer "
