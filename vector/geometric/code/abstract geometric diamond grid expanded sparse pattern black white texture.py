...
def plot_dot_wrapped(ax, cx, cy, **kwargs):
    for dx in (-PERIOD, 0.0, PERIOD):
        for dy in (-PERIOD, 0.0, PERIOD):
            ax.plot(cx + dx, cy + dy, **kwargs)
...
def diamond_grid_expanded_sparse():
    """Teselasi belah ketupat jarang beraksen titik, full canvas, seamless."""
    fig, ax = setup_ax()
