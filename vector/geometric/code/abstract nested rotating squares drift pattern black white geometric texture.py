...
def nested rotating squares drift():
    ...
    for i in range(n squares):
        ...
        max drift = min(12, 40 - size)  # Ensure squares don't go outside canvas
        cx = 50 + max drift * t * 0.5  # Reduced drift amount
        cy = 50 + max drift * t * 0.3  # Different drift rates for x and y
        angle = i * 5.0
