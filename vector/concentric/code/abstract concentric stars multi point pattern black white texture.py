...
def draw():
    ...
    for i in range(1, n_shapes + 1):
        r_outer = max_radius * (i / n_shapes)
        r_inner = r_outer * 0.45  # Inner radius of the star
        
        n_points = 8
        angles = np.linspace(0, 2*np.pi, n_points * 2 + 1)
        
        # Alternate between outer and inner radius
        radii = np.where(np.arange(len(angles)) % 2 == 0, r_outer, r_inner)
    ...
    save(fig, "abstract concentric stars multi point pattern black white texture")
